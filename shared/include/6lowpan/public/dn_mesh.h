...
// Dust epoch: seconds since 20:00 07/02/2002 PST
...
typedef struct {
   dn_asn_t  asn;
   INT16U    offset;    //ASN offset by micro-seconds
}  dn_asn_time_t;

// Security key 
typedef INT8U  dn_key_t[DN_KEY128_SIZE];

// MAC/Mesh layer MAC address
typedef INT8U  dn_macaddr_t[DN_MACADDR_SIZE];

typedef  struct {
   INT8U  byte[DN_IPV6_ADDR_SIZE];
}  dn_ipv6_addr_t;

// network id
typedef INT16U dn_netid_t;
